...
def _allele_for_realmccoil(
    datastore, allele_idxes, nonhet_masks=None, nonfailed_masks=None
):
...
def _allele_for_hmmibd(
    ...
    if nonhet_masks is not None:
        ...
        raise ValueError(
            "hmmIBD can not process het alleles, please use majority option"
        )
...
def get_alleles(
    func, dataset, hetratio=0.67, mindepth=5, minaltdepth=2, useGT=False, threads=-1
):
    ...
    if hetratio > 0 or minaltdepth > 0:
        ...
        if minaltdepth > 0:
            ...
            nonhetminalt_masks = (
                (total_depths - highest_depths) < minaltdepth
            ).compute()
