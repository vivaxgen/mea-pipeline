import os
from mea_pipeline import cerr, cexit
import pandas as pd
import numpy as np
...
def vcf_to_zarr(
    vcffile, outpath, fields=["INFO/*", "FORMAT/*"], max_alt_alleles=3, ploidy=2
):
...
def load_dataset(inpath, max_alt_alleles=8, ploidy=2, fields=["INFO/*", "FORMAT/*"]):
    ...
    if inpath.lower().endswith(".vcf") or inpath.lower().endswith(".vcf.gz"):
        return read_vcf(
            inpath, fields=fields, max_alt_alleles=max_alt_alleles, ploidy=ploidy
        )
...
def read_vcf(
    path, fields=["INFO/*", "FORMAT/*"], max_alt_alleles=3, ploidy=2, outpath=None
):
...
def get_position_tuples(ds):
    ...
    return zip(
        np.array(ds.contig_id)[ds.variant_contig.values], ds.variant_position.values
    )
...
def select_samples(ds, *, samples=[], samplefile=None):
...
def prepare_dataset(
    ...
    *,
    ...
    from mea_pipeline.utils import posutils, tabutils
    ...
    if not isinstance(ds_or_path, xr.core.dataset.Dataset):
        ...
        ds = load_dataset(
            ds_or_path,
            fields=["INFO/*", "FORMAT/GT", "FORMAT/AD", "FORMAT/DP"],
            max_alt_alleles=max_alt_alleles,
            ploidy=ploidy,
        )
    ...
    if not duplicate_position_allowed and has_duplicate_positions(ds):
        cexit(
            "ERROR: duplicate positions found! "
